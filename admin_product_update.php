...
<div class="form-group">
  ...
  <div class="col-md-4">
  <input id="p_name" name="p_name" type="text" placeholder="ชื่อสินค้า" class="form-control input-md" required value="<?=$data['p_name'];?>">
  ...
  <div class="col-md-4">
	...
	<?php
		if(isset($_POST['Submit'])){
			...
			include("connectdb.php");
			$sql = "UPDATE `product` SET `p_id` = '{$_POST['p_id']}', `p_name` = '{$_POST['p_name']}', `p_detail` = '{$_POST['p_detail']}', `p_price` = '{$_POST['p_price']}', `p_picture` = '{$_POST['p_picture']}', `p_type` = '{$_POST['p_type']}', `p_filename` = '{$_POST['p_filename']}' WHERE `product`.`p_id` = {$_GET['id']};";
